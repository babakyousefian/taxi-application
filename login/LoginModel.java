...
public class LoginModel extends ViewModel {
    ...
    /*call the Api function authDriverTelNumber and define a callback for this Api*/
    public void authenticateDriver(String countryCode, String mobileNumber) {
        Call<ResponseBody> driverApiCall = mDriverApiService.getDriverApi().authDriverTelNumber(countryCode, mobileNumber);
        //define the callback authenticateDriverCallback to set the appropriate live data when
        // the Api function returns
        driverApiCall.enqueue(new authenticateDriverCallback());
    }
    ...
    /*Callback for authenticateDriver function*/
    private class authenticateDriverCallback implements Callback<ResponseBody> {
    ...
    private class verifyDriverCallback implements Callback<DriverResponse> {
        ...
        public void onResponse( Call<DriverResponse> call, Response<DriverResponse> response) {
            ...
            switch (RetCode)
            {
                ...
                case 404: //resource not found
                    setRespVerifyDriver(LoginActivity.getContext().getString(R.string.verification_code_not_correct));
                    Log.d(TAG, response.message());
                    break;
                case 422: //validation error
